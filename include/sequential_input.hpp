#ifndef SPANET_ONNX_SEQUENTIAL_INPUT_HPP
#define SPANET_ONNX_SEQUENTIAL_INPUT_HPP
...
public:
    SequentialInput(
        ...
        mask = H5Easy::load<xt::xtensor<bool, 2>>(file, "INPUTS/" + name + "/MASK");
        ...
        for (auto i = 0; i < feature_names.size(); ++i) {
            ...
            const auto data = H5Easy::load<xt::xtensor<float, 2>>(file, feature_key(feature_name));

            auto current_feature = xt::view(features, xt::all(), xt::all(), i);
...
#endif //SPANET_ONNX_SEQUENTIAL_INPUT_HPP
